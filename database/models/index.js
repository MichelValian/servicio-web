'use strict';

const fs = require('fs');
const path = require('path');
const Sequelize = require('sequelize');
const process = require('process');
const basename = path.basename(__filename);
const env = process.env.NODE_ENV || 'development';
const config = require(__dirname + '/../config/config.json')[env];
const db = {};

let sequelize;
if (config.use_env_variable) {
  sequelize = new Sequelize(process.env[config.use_env_variable], config);
} else {
  sequelize = new Sequelize(config.database, config.username, config.password, config);
}

// import cobro from './cobro';
// import cobrorango from './cobrorango';
// import customer from './customer';
// import autor from './autor';
// import autorCategory from './autorCategory';
// import document from './document';
// import user from './user'
import state from './state'
import city from './city'
import locality from './locality'
import address from './address'



/*
fs
  .readdirSync(__dirname)
  .filter(file => {
    return (
      file.indexOf('.') !== 0 &&
      file !== basename &&
      file.slice(-3) === '.js' &&
      file.indexOf('.test.js') === -1
    );
  })
  .forEach(file => {
    const model = require(path.join(__dirname, file))(sequelize, Sequelize.DataTypes);
    db[model.name] = model;
  });
*/

// db.Cobro = cobro(sequelize, Sequelize.DataTypes);
// db.CobroRango = cobrorango(sequelize, Sequelize.DataTypes);
// db.Customer = customer(sequelize, Sequelize.DataTypes);
// db.Autor = autor(sequelize, Sequelize.DataTypes);
// db.AutorCategory = autorCategory(sequelize, Sequelize.DataTypes);
// db.Document = document(sequelize, Sequelize.DataTypes);
// db.User = user(sequelize, Sequelize.DataTypes);
db.State = state(sequelize, Sequelize.DataTypes);
db.City = city(sequelize, Sequelize.DataTypes);
db.Locality = locality(sequelize, Sequelize.DataTypes);
db.Address = address(sequelize, Sequelize.DataTypes);


Object.keys(db).forEach(modelName => {
  if (db[modelName].associate) {
    db[modelName].associate(db);
  }
});


db.sequelize = sequelize;
db.Sequelize = Sequelize;

module.exports = db;
